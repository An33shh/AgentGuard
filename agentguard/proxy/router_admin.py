...
blocked forever, with no automatic recovery. These endpoints are the
operator-visible way to see and clear that state without restarting the
whole proxy — see project memory: the 2026-08-11 dogfood incident, where a
demoted/locked-out session had no visible cause or recovery path short of a
full process restart.

The counters used to live only in the handling process's memory, invisible
to any other replica (including the dashboard's separate API process) —
Interceptor is now backed by SessionTracker (agentguard/interceptor/
session_tracker.py), which shares this state via Redis when REDIS_URL is
configured, so these endpoints see the same counters every replica enforces
against, not just whichever one happened to handle this request.
...
async def get_session(session_id: str) -> JSONResponse:
    ...
    return JSONResponse(await interceptor.get_session_stats(session_id))
