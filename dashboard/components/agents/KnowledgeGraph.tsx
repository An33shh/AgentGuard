...
function NodeInfoPanel({ node, onClose }: { node: NodeCanvasObj; onClose: () => void }) {
  ...
  return (
    <div className="absolute bottom-10 right-4 z-20 w-72 bg-[#0C1220]/95 backdrop-blur-md rounded-xl border border-[#243354] shadow-2xl p-4 text-sm">
      <div className="flex items-center justify-between mb-2">
        <span className="text-xs uppercase tracking-widest font-bold" style={{ color }}>
          {TYPE_LABELS[node.type] ?? node.type}
        </span>
        <button onClick={onClose} className="text-[#484F58] hover:text-[#E6EDF3] transition-colors leading-none">✕</button>
      </div>

      <p className="font-semibold text-[#E6EDF3] mb-3 leading-snug">{node.label ?? node.id}</p>

      {node.type === "agent" && (
        <div className="space-y-2 text-xs text-[#8B949E]">
          <p>The AI agent being monitored. All sessions, tools used, and attack patterns it triggered branch from this node.</p>
          <p className="font-mono text-[#484F58] break-all">{node.agent_id}</p>
          <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${
            node.is_registered ? "bg-indigo-900/60 text-indigo-300" : "bg-[#101828] text-[#484F58]"
          }`}>
      ...
      {node.type === "pattern" && (
        <div className="space-y-2 text-xs">
          <p className="text-[#F85149] font-semibold">Detected attack pattern</p>
      ...
      <p className="text-[#3A4A5C] text-xs mt-3 italic">Click the same node again to dismiss.</p>
    </div>
  );
}
...
const ForceGraph2D = dynamic(() => import("react-force-graph-2d") as Promise<{ default: React.ComponentType<any> }>, {
  ssr: false,
  loading: () => (
    <div className="flex items-center justify-center bg-[#070B14] rounded-xl" style={{ height: 500 }}>
      <p className="text-[#484F58] text-sm animate-pulse">Loading graph…</p>
    </div>
  ),
});
  ...
  const content = (
    <div
      ...
      className={
        fullscreen
          ? "fixed inset-0 z-50"
          : "relative w-full rounded-xl overflow-hidden border border-[#1C2844]"
      }
    >
      {data.nodes.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 bg-[#070B14] gap-2 rounded-xl">
          <p className="text-[#484F58] text-sm">No graph data yet.</p>
          <p className="text-[#3A4A5C] text-xs font-mono">python examples/demo_attack.py</p>
        </div>
      ) : (
        <>
          ...
          <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-4 py-3">
            ...
            <button
              ...
              className="text-[#484F58] hover:text-[#E6EDF3] transition-colors p-1.5 rounded-lg hover:bg-white/10 backdrop-blur-sm"
          ...
          <div className="absolute top-12 right-4 z-10 text-right pointer-events-none select-none">
            <p className="text-xs text-[#3A4A5C]">Scroll to zoom · Drag to pan</p>
            <p className="text-xs text-[#3A4A5C]">Hover to highlight · Click to inspect</p>
          </div>
          ...
          {/* ── Stats ── */}
          <div className="absolute bottom-3 left-4 z-10 text-xs text-[#3A4A5C] pointer-events-none select-none">
